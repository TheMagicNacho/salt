...
class MainWindow {
    ...
    HWND Create(HINSTANCE hInstance, int nCmdShow) {
        hwnd_ =
            CreateWindowExW(0, kClassName, L"Untitled - Salt Text Editor", WS_OVERLAPPEDWINDOW,
                            CW_USEDEFAULT, CW_USEDEFAULT, 1000, 680, NULL, NULL, hInstance, this);
   ...
   private:
    ...
    void RecreateEditControl() {
        ...
        edit_hwnd_ = CreateWindowExW(0, L"EDIT", buffer.c_str(), style, 0, 0, rcClient.right,
                                     rcClient.bottom, hwnd_, reinterpret_cast<HMENU>(ControlId::MainEdit),
        ...
        switch (uMsg) {
            ...
            case WM_SIZE: {
                ...
                if (status_hwnd_ && options_handler_.IsStatusBarVisible()) {
                    SendMessageW(status_hwnd_, WM_SIZE, wParam, lParam);
                    RECT rcStatus;
            ...
            case WM_CTLCOLORSTATIC: {
                if (reinterpret_cast<HWND>(lParam) == edit_hwnd_) {
            ...
            case WM_COMMAND: {
                ...
                if (HIWORD(wParam) == EN_CHANGE && reinterpret_cast<HWND>(lParam) == edit_hwnd_) {
                    if (!file_handler_.IsDirty()) {
                ...
                Command cmd = static_cast<Command>(LOWORD(wParam));
                switch (cmd) {
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    ...
    HACCEL hAccel = MainWindow::CreateAppAccelerators();
